...
def test_deliver_copies_package_and_records(tmp_path: Path):
    ...
    result = runner.invoke(cli.app, ["--config", cfg, "deliver", str(sws.dir), "--dest", str(dest)])
    assert result.exit_code == 0, result.output
...
def test_deliver_refuses_needs_review_without_force(tmp_path: Path):
    ...
    forced = runner.invoke(cli.app, ["--config", cfg, "deliver", str(sws.dir), "--dest", str(dest),
                                     "--force"])
...
def test_deliver_batch_continues_past_oserror(tmp_path, monkeypatch):
    ...
    r = runner.invoke(cli.app, ["--config", cfg, "deliver", "--packaged", "--yes"])
    assert r.exit_code == 0, r.output
...
def test_deliver_rejects_name_and_selector_together(tmp_path):
    cfg = str(tmp_path / "config.toml")
    (tmp_path / "config.toml").write_text(f'root = "{tmp_path}"\n')
    r = runner.invoke(cli.app, ["--config", cfg, "deliver", "someshow", "--packaged"])
    assert r.exit_code != 0
    assert "not both" in r.output.lower()
...
def test_deliver_by_name_records_provenance_run(tmp_path: Path):
    ...
    dest = tmp_path / "inbox"
    result = runner.invoke(cli.app, ["--config", cfg, "deliver", "aaa", "--dest", str(dest)])
    assert result.exit_code == 0, result.output
